...
def randtopo( num , low , high ):
	...
	while i<num:
		...
		g.new('')
		randfill(sel,(1+random.randrange(area))*int(100/area),mixpercent)
		if g.empty():
			continue
		h=g.hash(g.getrect())
		if h in hashlist:
...
input=g.getstring('cyclenum/toponum/sizelist/bool_rand1_read0','5/10000/3,4/1/50')
